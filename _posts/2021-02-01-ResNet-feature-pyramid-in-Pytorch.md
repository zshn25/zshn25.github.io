...
description: "Tutorial on how to get feature pyramids from Pytorch's ResNet models"
image: images/image_pyramid.webp
date: 2021-02-09 15:21:23 -0700
categories: deep-learning computer-vision pytorch
author: Zeeshan Khan Suri
published: true
comments: true
nb_path: _notebooks/2021-02-01-ResNet-feature-pyramid-in-Pytorch.ipynb
---

[![image pyramid](https://upload.wikimedia.org/wikipedia/commons/thumb/4/43/Image_pyramid.svg/1200px-Image_pyramid.svg.png "Image Pyramid. By Cmglee, CC BY-SA 3.0")](https://commons.wikimedia.org/wiki/File:Image_pyramid.svg#/media/File:Image_pyramid.svg)
...
[![u-net](https://upload.wikimedia.org/wikipedia/commons/2/2b/Example_architecture_of_U-Net_for_producing_k_256-by-256_image_masks_for_a_256-by-256_RGB_image.png "U-Net uses its encoder feature pyramid as skip connections to the decoder. Image by Mehrdad Yazdani, CC BY-SA 4.0")](https://en.wikipedia.org/wiki/U-Net#/media/File:Example_architecture_of_U-Net_for_producing_k_256-by-256_image_masks_for_a_256-by-256_RGB_image.png)
...
That's it. Now, to use it, add it to the source of ResNet. I have made a Gist on how to do this [here](https://gist.github.com/zshn25/09683dadb6b4e60e8382a380020d144e/). Now you can give an argument `features_only` and it will return the feature pyramid as defined above.
...
```python
## This file is a modified version of [Pytorch's ResNet](https://github.com/pytorch/vision/blob/master/torchvision/models/resnet.py) and inherits it's licence.
...
__all__ = ['ResNet', 'ResNetFeatures', 'resnet18', 'resnet34', 'resnet50', 'resnet101',
...
def _resnet(
    ...
    **kwargs: Any
    ...
    if features_only:
        model = ResNetFeatures(block=block, layers=layers, **kwargs)
    else:
...
```python
model = resnet18(pretrained=True, features_only=True)

features = model(torch.randn((1,3,265,265)))

[print(f.shape) for f in features]
```

    torch.Size([1, 64, 133, 133])
    torch.Size([1, 64, 67, 67])
    torch.Size([1, 128, 34, 34])
...
```python
@misc{suri_ResNet-feature-pyramid-in-Pytorch_2021,
      title={ResNet feature pyramid in Pytorch},
